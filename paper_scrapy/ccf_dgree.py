...
for j in ['A']:
    for i in hrefs:
        ...
        for journal in select_data:
            ...
            # 直接通过XPath查找元素
            li_text = tree.xpath('//*[@id="info-section"]/div/div/ul/li[1]/text()')
            # print(li_text)
            if not li_text:
                print("没有定位到元素")
                # print(journal['address'])
                ...
                if li_text[0].strip() == '':
                    ...
                    Journal_Articles[j].append(li_text[0])
                    # 循环遍历并将每个期刊名称追加到文本文件中
                    with open('Journal_Articles_A.txt', 'a') as file:
                        file.write(li_text[0] + ',')
                    print(li_text[0])


for k in ['A']:
    print(f'{k}类所有期刊名: \n')
    print(Journal_Articles[k])
    json_data = json.dumps(Journal_Articles)
    # 将JSON字符串写入文件
    with open('Journal_Articles_A.json', 'w') as file:
