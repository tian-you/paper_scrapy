...
def process_function(journal, proxy, journal_name, start_year, end_year, keyword):
    print(f"Process ID: {os.getpid()}")  # 打印进程ID
    journal_url = journal['address']
    titles_and_journals, proxy = extract_volumes(journal_url, proxy, journal_name, start_year, end_year, keyword)
    return titles_and_journals, proxy
...
def main():
    ...
    all_titles = []
    # proxy = 'your_initial_proxy'  # 初始代理设置
    # user_agents = [...]  # 用户代理列表
    ...
    with Pool(processes=10) as pool:
        ...
        for journal in select_data:
            ...
            # 提交任务到进程池
            result = pool.apply_async(process_function, (journal, current_proxy, journal_name, start_year, end_year, keyword))
            results.append(result)

        # 收集所有进程的结果
        for result in results:
            try:
                titles_and_journals, proxy = result.get()  # get()会阻塞直到有结果返回
                all_titles.extend(titles_and_journals)
            except Exception as e:
                print(f"Process resulted in an exception: {e}")

    sum_article = len(all_titles)
    print(f"Total number of articles containing the keyword: {sum_article}")
